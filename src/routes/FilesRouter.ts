...
router.post('/', fileLimiter, UploadMiddleware, upload.single('file'), async (req: Request, res: Response) => {
    ...
    if (!allowedFileTypes.some((fileType: string) => file.filename.toLowerCase().endsWith(`.${fileType}`))) return res.sendStatus(403).json({
        success: false,
        error: `The allowed file types are ${allowedFileTypes.map((fileType: string) => `.${fileType}`).join(', ').replace(/,\s([^,]+)$/, ', and $1')}`
    });
...
router.delete('/:id', AuthMiddleware, async (req: Request, res: Response) => {
    ...
    if (user._id != file.uploader.uuid) return res.status(404).json({
        success: false,
        error: 'not your file',
    });
