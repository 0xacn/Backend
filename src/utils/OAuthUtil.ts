...
export class OAuth {
    ...
    request = async (endpoint: string, method: Method, body?: object | string, headers?: object): Promise<any> => {
        try {
            ...
            throw new Error(err.response.data.error_description || err.response.data.message
            );
        }
    }
    ...
    addGuildMember = async (user: User): Promise<void> => {
        ...
        if (user.discord.id && user.discord.id !== this.user.id) {
            try {
                ...
                await this.request(`/guilds/${process.env.DISCORD_SERVER_ID}/members/${this.user.id}`, 'PATCH',
                    data, {
                        'Authorization': `Bot ${process.env.DISCORD_BOT_TOKEN}`,
                    }
                );
