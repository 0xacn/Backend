...
const lookup = require('safe-browse-url-lookup')({apiKey: 'AIzaSyAv7Ir63bdIx03t5WVJzZlnhYUtKBrDFUQ'});
const check = new proxy_check({api_key: '71x722-t44jz9-7043n4-13nb77'});

//this is hella ugly IK
async function isVPN(ip: string) {
    var ipintel = await Axios.get(`http://check.getipintel.net/check.php?ip=${ip}&contact=hello@clippy.gg`, {transformResponse: []});
