...
async function request(
  ...
  try {
    ...
    throw new Error(
      "Couldn't link your discord. Please make sure you are not in the 100 server limit"
    );
  }
}
...
async function addPremium(user: User) {
  await this.request(
    `/guilds/${process.env.DISCORD_SERVER_ID}/members/${user.discord.id}/roles/806106770212126730`,
    'PUT',
    null,
    {
...
export {s3, wipeFiles, updateStorage, addPremium, request};
